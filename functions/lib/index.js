...
exports.onCommentsChange = functions.firestore.document('requests/{requestId}/comments/{commentId}').onUpdate((change, context) => __awaiter(this, void 0, void 0, function* () {
    const newValue = change.after.data();
    const previousValue = change.before.data();
    //comment set right
    if (!previousValue.right && newValue.right) {
        yield actWithRating(newValue.user);
        ...
        //     user:
        // });
        return admin.firestore().doc('requests/' + context.params.requestId).collection('comments').get().then((querySnapshot) => {
            const documents = [];
            ...
            return Promise.all(documents.map((doc) => __awaiter(this, void 0, void 0, function* () { return doc.ref.set({ right: false }, { merge: true }); })));
        }).catch(e => console.log(e));
    }
    return 'ok';
}));
exports.onNewComments = functions.firestore.document('requests/{requestId}/comments/{commentId}').onCreate((snapshot, context) => {
    return admin.firestore().doc('requests/' + context.params.requestId).get().then((requestDoc) => __awaiter(this, void 0, void 0, function* () {
        const comment = snapshot.data();
        const authorSnap = yield comment.user.get();
        const author = Object.assign({}, authorSnap.data());
        ...
        yield request.user.collection('notifications').add({
            user: comment.user,
        ...
        yield actWithRating(request.user);
...
exports.onRequestLike = functions.firestore.document('requests/{requestId}/comments/{commentId}/likes/{likeId}').onCreate((snapshot, context) => {
    ...
    return admin.firestore().doc('requests/' + context.params.requestId + '/comments/' + context.params.commentId).get().then((commentSnap) => __awaiter(this, void 0, void 0, function* () {
        const comment = commentSnap.data();
        const authorSnap = yield like.user.get();
        const author = Object.assign({}, authorSnap.data());
        const payload = {
            notification: {
                title: 'Новый лайк!',
        ...
        yield comment.user.collection('notifications').add({
            user: like.user,
            title: payload.notification.body,
            createdAt: +Date.now(),
            meta: {
        ...
        yield actWithRating(comment.user);
...
function setNotificationToUser(userRef, payload) {
...
function actWithRating(userRef) {
    return __awaiter(this, void 0, void 0, function* () {
        //полезные отклики
        const userSnap = yield userRef.get();
        const user = userSnap.data();
        const allResponds = yield userRef.collection('responds').get();
        let rightRespondsCount = 0;
        const responds = [];
        allResponds.forEach((item) => responds.push(item));
        yield Promise.all(responds.map((item) => __awaiter(this, void 0, void 0, function* () {
            const resp = item.data();
            const respDoc = yield resp.respond.get();
        ...
        //лайки на отклики
        const likesSnap = yield admin.firestore().collection('requests/comments/likes').where('user', '==', userRef).get();
        const likesCount = likesSnap.size;
        //собственные запросы
        const requestsSnap = yield admin.firestore().collection('requests')
            .where('type', '==', 'request')
            .where('user', '==', userRef)
            .get();
        ...
        //собственные новости
        const newsSnap = yield admin.firestore().collection('requests')
            .where('type', '==', 'news')
            .where('user', '==', userRef)
            .get();
        ...
        //собственные лоты
        const lotsSnap = yield admin.firestore().collection('requests')
            .where('type', '==', 'lot')
            .where('user', '==', userRef)
            .get();
        const lotsCount = lotsSnap.size;
        //собственные партнерства
        const partSnap = yield admin.firestore().collection('requests')
        ...
        if (diff > 0.01) {
            yield userRef.collection('rating').add({
                value: diff,
                createdAt: +Date.now()
            });
        }
